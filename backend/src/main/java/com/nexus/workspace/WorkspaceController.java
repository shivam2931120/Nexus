...
public class WorkspaceController {
    ...
    public WorkspaceController(JdbcTemplate db, OrganizationController organizations) { this.db = db; this.organizations = organizations; }
    ...
    public Map<String,Object> bootstrap(org.springframework.security.core.Authentication auth) {
        UUID uid = user(auth);
        ...
        if (memberships.isEmpty()) {
            ...
            String slug = "my-workspace-" + orgId.toString().substring(0, 8);
            db.update("INSERT INTO org.organizations(id,name,slug) VALUES (?,?,?)", orgId, orgName, slug);
            db.update("INSERT INTO org.memberships(id,organization_id,user_id,role) VALUES (?,?,?,?)", UUID.randomUUID(), orgId, uid, "OWNER");
            role = "OWNER";
            audit(orgId, uid, "workspace.created", "organization", orgId);
        } else {
            Map<String,Object> row = memberships.get(0);
            orgId = (UUID) row.get("id"); orgName = (String) row.get("name"); role = (String) row.get("role");
        }
        List<Map<String,Object>> teams = db.queryForList("SELECT id,name,description FROM org.teams WHERE organization_id=? AND deleted_at IS NULL ORDER BY created_at", orgId);
        UUID teamId;
        if (teams.isEmpty()) {
            teamId = UUID.randomUUID();
            db.update("INSERT INTO org.teams(id,organization_id,name,description) VALUES (?,?,?,?)", teamId, orgId, "General", "Default workspace team");
            db.update("INSERT INTO org.team_members(team_id,user_id) VALUES (?,?)", teamId, uid);
            db.update("INSERT INTO chat.channels(id,organization_id,team_id,name,type) VALUES (?,?,?,?,?)", UUID.randomUUID(), orgId, teamId, "general", "PUBLIC");
        ...
        List<Map<String,Object>> channels = db.queryForList("SELECT id,name,type,team_id FROM chat.channels WHERE organization_id=? AND deleted_at IS NULL ORDER BY name", orgId);
        return Map.of("user", db.queryForMap("SELECT id,email,name FROM nexus_auth.users WHERE id=?", uid), "organization", Map.of("id", orgId, "name", orgName, "role", role), "team", teams.get(0), "channels", channels);
    }
    ...
    @PostMapping("/orgs/{orgId}/invitations") public Map<String,Object> invite(@PathVariable UUID orgId,@Valid @RequestBody CreateInvitation r,org.springframework.security.core.Authentication a){UUID uid=user(a);admin(orgId,uid);UUID id=UUID.randomUUID();db.update("INSERT INTO org.invitations(id,organization_id,email,role,invited_by) VALUES(?,?,?,?,?)",id,orgId,r.email(),Objects.requireNonNullElse(r.role(),"MEMBER"));db.update("INSERT INTO notification.notifications(id,organization_id,user_id,type,title,body) VALUES(?,?,?,?,?,?)",UUID.randomUUID(),orgId,uid,"INVITATION","Invitation created","Invitation created for "+r.email());audit(orgId,uid,"invitation.created","invitation",id);return db.queryForMap("SELECT id,email,role,status,created_at,expires_at FROM org.invitations WHERE id=?",id);}
