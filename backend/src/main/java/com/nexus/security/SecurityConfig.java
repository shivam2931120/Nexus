...
class ClerkJwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest req,HttpServletResponse res,FilterChain chain)throws ServletException,IOException{
        ...
        var h=req.getHeader("Authorization"); if(decoder!=null&&h!=null&&h.startsWith("Bearer ")){try{var token=decoder.decode(h.substring(7));var sub=token.getSubject();var u=users.findByClerkId(sub).orElseGet(()->users.findByEmailIgnoreCase(claim(token,"email")).orElseGet(()->users.save(new User(claim(token,"email"),"{clerk}"+java.util.UUID.randomUUID(),displayName(token),sub))));var auth=new UsernamePasswordAuthenticationToken(u.getId().toString(),null,List.of(new SimpleGrantedAuthority("ROLE_USER")));org.springframework.security.core.context.SecurityContextHolder.getContext().setAuthentication(auth);}catch(Exception ignored){}}
