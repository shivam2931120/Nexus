...
export default function FilesPage() {
  ...
  const [folders,setFolders]=useState<FolderItem[]>([]); const [folderId,setFolderId]=useState<string|null>(null); const [folderName,setFolderName]=useState(''); const [versions,setVersions]=useState<Version[]>([]); const [previewUrl,setPreviewUrl]=useState(''); const [notice,setNotice]=useState('')
  ...
  useEffect(() => { void load().catch(() => {}) }, [orgId,folderId])
  ...
  const createFolder=async()=>{if(!orgId||!folderName.trim())return;const item=await api<FolderItem>(`/orgs/${orgId}/folders`,{method:'POST',body:JSON.stringify({name:folderName.trim(),parentId:folderId,teamId})});setFolders(current=>[...current,item]);setFolderName('')}
  const inspect=async(file:FileItem)=>{setSelected(file);setPreviewUrl('');try{const [history,download]=await Promise.all([api<Version[]>(`/files/${file.id}/versions`),api<{url:string}>(`/files/${file.id}/download`)]);setVersions(history);if(file.mime_type.startsWith('image/')||file.mime_type==='application/pdf')setPreviewUrl(download.url)}catch{setVersions([])}}
  const share=async(file:FileItem)=>{const link=await api<{path:string}>(`/files/${file.id}/share`,{method:'POST',body:JSON.stringify({expiresAt:new Date(Date.now()+7*86400000).toISOString(),maxDownloads:100})});const url=`${window.location.origin}${link.path}`;await navigator.clipboard.writeText(url);setNotice('A seven-day shared link was copied to the clipboard.')}
