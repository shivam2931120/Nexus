...
function WorkspaceHome() {
  ...
  const [data, setData] = useState<{ tasks: Array<{ id: string; title: string; status: string; due_date?: string }>; meetings: Array<{ id: string; title: string; scheduled_at?: string }>; documents: Array<{ id: string; title: string; updated_at: string }>; messages: Array<{ id: string; content: string; sender_name: string; channel_name: string }> } | null>(null)
  const [error, setError] = useState('')
  useEffect(() => { if (orgId) void api<typeof data>(`/orgs/${orgId}/dashboard`).then(setData).catch(err => setError(err instanceof Error ? err.message : 'Dashboard could not be loaded.')) }, [orgId])
  return <AppShell><div className="page"><div className="page-heading"><div><div className="eyebrow">WORKSPACE / HOME</div><h1>Workspace dashboard</h1><p className="muted">A live view of work, meetings, documents, and team activity.</p></div><div className="actions"><Link className="button primary" href="/tasks?new=1">New task</Link><Link className="button" href="/documents?new=1">New document</Link><Link className="button" href="/meetings?new=1">Schedule meeting</Link></div></div>{error && <div className="form-error">{error}</div>}<div className="grid dashboard-grid"><section className="card"><div className="card-header"><h2>My work</h2><Link className="button" href="/tasks">View tasks</Link></div>{data?.tasks?.length ? data.tasks.map(task => <div className="task-row" key={task.id}><span className={`status-dot ${task.status === 'DONE' ? 'done' : 'progress'}`} /><div className="task-copy"><strong>{task.title}</strong><small>{task.status}{task.due_date ? ` · due ${new Date(task.due_date).toLocaleDateString()}` : ''}</small></div></div>) : <div className="empty"><p>{data ? 'No assigned tasks yet.' : 'Loading assigned work…'}</p><Link className="button primary" href="/tasks?new=1">Create task</Link></div>}</section><section className="card"><div className="card-header"><h2>Upcoming meetings</h2><Link className="button" href="/calendar">Calendar</Link></div>{data?.meetings?.length ? data.meetings.map(meeting => <div className="task-row" key={meeting.id}><span className="status-dot progress" /><div className="task-copy"><strong>{meeting.title}</strong><small>{meeting.scheduled_at ? new Date(meeting.scheduled_at).toLocaleString() : 'Room ready'}</small></div><Link className="button" href={`/meetings/room?meetingId=${meeting.id}`}>Join</Link></div>) : <div className="empty"><p>{data ? 'No upcoming meetings.' : 'Loading meetings…'}</p><Link className="button primary" href="/meetings?new=1">Schedule one</Link></div>}</section><section className="card"><div className="card-header"><h2>Recent documents</h2><Link className="button" href="/documents">Open docs</Link></div>{data?.documents?.length ? data.documents.map(document => <div className="task-row" key={document.id}><div className="task-copy"><strong>{document.title}</strong><small>Updated {new Date(document.updated_at).toLocaleDateString()}</small></div></div>) : <div className="empty"><p>{data ? 'No documents yet.' : 'Loading documents…'}</p><Link className="button" href="/documents?new=1">Create document</Link></div>}</section><section className="card"><div className="card-header"><h2>Team activity</h2><Link className="button" href="/chat">Open chat</Link></div>{data?.messages?.length ? data.messages.slice(0, 5).map(message => <div className="task-row" key={message.id}><div className="task-copy"><strong>{message.sender_name} · #{message.channel_name}</strong><small>{message.content}</small></div></div>) : <div className="empty"><p>{data ? 'No recent messages.' : 'Loading activity…'}</p></div>}</section></div></div></AppShell>
}
