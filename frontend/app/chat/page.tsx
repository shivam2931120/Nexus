...
export default function Chat() {
  ...
  useEffect(() => {
    if (!active) return
    void api<Message[]>(`/channels/${active.id}/messages/enriched`).then(items => { setMessages(items.map(item => item.sender_name === 'Clerk user' ? { ...item, sender_name: currentName } : item)); void api(`/channels/${active.id}/read`, { method: 'PUT' }) }).catch(err => setError(err instanceof Error ? err.message : 'Messages could not be loaded.'))
  }, [active, currentName])
  ...
  const react = async (message: Message, emoji = '👍') => { if (!active) return; try { await api(`/channels/${active.id}/messages/${message.id}/reactions`, { method: 'POST', body: JSON.stringify({ emoji }) }); setMessages(items => items.map(item => item.id === message.id ? { ...item, reactions: { ...item.reactions, [emoji]: (item.reactions?.[emoji] ?? 0) + 1 } } : item)) } catch (err) { setError(err instanceof Error ? err.message : 'Reaction could not be saved.') } }
