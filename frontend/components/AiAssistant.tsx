...
export default function AiAssistant({ title = 'AI Assistant', context = '', compact = false }: Props) {
  ...
  return <section className={`card ai-assistant ${compact ? 'ai-assistant-compact' : ''}`}>
    <div className="card-header"><div><div className="eyebrow">NEMOTRON / WORKSPACE INTELLIGENCE</div><h2>{title}</h2></div><Sparkles size={18} /></div>
    ...
    <form className="ai-form" onSubmit={submit}><input value={message} onChange={event => setMessage(event.target.value)} placeholder="Ask Nemotron…" aria-label="Ask AI assistant" /><button className="button primary" disabled={busy || !message.trim()}>{busy ? 'Thinking…' : 'Ask'}</button></form>
  </section>
}
