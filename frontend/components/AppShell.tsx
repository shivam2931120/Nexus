...
import { Activity, Bell, BookOpen, CalendarDays, CircleHelp, FileText, FolderKanban, Home, Inbox, ListChecks, ListTodo, LogOut, Menu, MessageCircle, Moon, Search, Settings, Sparkles, Sun, Users, Video, X } from './icons'
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const [name, setName] = useState('Nexus user'); const [dark, setDark] = useState(true); const [mobileNavOpen, setMobileNavOpen] = useState(false); const [unread, setUnread] = useState(0); const [searchOpen, setSearchOpen] = useState(false); const [serverError, setServerError] = useState(''); const [q, setQ] = useState(''); const [results, setResults] = useState<Result[]>([])
  ...
  return <div className={`shell ${mobileNavOpen ? 'mobile-nav-open' : ''}`}><header className="topbar"><button className="icon-button mobile-menu-button" aria-label={mobileNavOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen(open => !open)}>{mobileNavOpen ? <X size={18} /> : <Menu size={18} />}</button><Link href="/" className="brand"><div className="brand-mark">N</div><span>Nexus</span></Link><button className="search" onClick={() => setSearchOpen(true)}><Search size={15} /><span>Search everything</span><kbd>⌘ K</kbd></button><div className="top-actions"><button aria-label="Help" className="icon-button" onClick={() => router.push('/knowledge')}><CircleHelp size={17} /></button><Link aria-label="Notifications" className="icon-button" href="/notifications"><Bell size={17} />{unread > 0 && <span className="nav-count">{unread > 99 ? '99+' : unread}</span>}</Link><button aria-label={dark ? 'Switch to light mode' : 'Switch to dark mode'} className="icon-button" onClick={toggle}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button><button aria-label="Open profile settings" title={name} className="avatar" onClick={() => router.push('/settings')}>{name.slice(0, 1).toUpperCase()}</button></div></header><aside className="sidebar"><nav>{nav(primary)}<div className="nav-section">TEAMS</div><Link href="/chat" className="nav-item"><MessageCircle size={17} /><span>Team channels</span></Link><div className="nav-section">WORKSPACE</div>{nav(workspace)}<div className="nav-section">ACCOUNT</div><Link href="/settings" className={`nav-item ${path === '/settings' ? 'active' : ''}`}><Settings size={17} /><span>Settings</span></Link><button className="nav-item" onClick={() => void logout()}><LogOut size={17} /><span>Sign out</span></button></nav></aside><main className="main">{children}</main>{searchOpen && <div className="modal-backdrop" onClick={() => setSearchOpen(false)}><div className="card modal-card" onClick={event => event.stopPropagation()}><div className="card-header"><h2>Search workspace</h2><button className="icon-button" aria-label="Close search" onClick={() => setSearchOpen(false)}><X size={17} /></button></div><div className="search section-gap"><Search size={15} /><input autoFocus value={q} onChange={event => setQ(event.target.value)} placeholder="Search tasks, documents, messages" /></div>{results.length === 0 ? <p className="muted section-gap">Type at least two characters to search.</p> : <div className="section-gap">{results.map(result => <button className="nav-item" key={`${result.type}-${result.id}`} onClick={() => { setSearchOpen(false); router.push(result.type === 'task' ? '/tasks' : result.type === 'document' ? '/documents' : '/chat') }}><Search size={15} /><span>{result.title ?? result.content}</span><small>{result.type}</small></button>)}</div>}</div></div>}{serverError && <div className="nexus-server-modal" role="alertdialog" aria-live="assertive"><div className="nexus-server-dialog"><div className="eyebrow">SYSTEM / REQUEST FAILED</div><h2>Server connection interrupted.</h2><p>{serverError}</p><div className="actions"><button className="button primary" onClick={() => window.location.reload()}>Retry page</button><button className="button" onClick={() => setServerError('')}>Dismiss</button></div></div></div>}</div>
}
